...
# Run the main code block multiple times (n=10)
n_runs = 2
for run_num in range(n_runs):
    ...
    random_angle_deg = random.uniform(-0.5, 0.01)
    ...
    # Stop AirSim recording after loop
    client.stopRecording()

    # Save data for this run (Modify this according to your data structure)
    run_data = {'steering_angles': steering_angles, 'yaw_list': yaw_list}
    run_data_df = pd.DataFrame(run_data)
    run_data_df.to_csv(os.path.join(recording_folder, f"run_{run_num + 1}_data.csv"), index=False)

    print(f"Run {run_num + 1}/{n_runs} completed.")
